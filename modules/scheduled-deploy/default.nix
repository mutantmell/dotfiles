...
let
  ...
  parseFlakeRef = flakeRef:
    let
      ...
      url = lib.head parts;
      ref = if lib.length parts > 1 then lib.elemAt parts 1 else null;
    in { inherit url ref; };
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = lib.mapAttrs' (nodeName: nodeConfig:
    ...
    systemd.timers = lib.mapAttrs' (nodeName: nodeConfig:
      lib.nameValuePair "scheduled-deploy-${nodeName}" {
