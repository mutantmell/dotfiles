...
pkgs.testers.nixosTest {
  ...
  nodes = {
    ...
    allowed = { config, pkgs, lib, ... }: {
      virtualisation.vlans = [ 1 ];
      networking = {
        useDHCP = false;
        interfaces.eth1.ipv4.addresses = [{ address = "192.168.1.20"; prefixLength = 24; }];
    ...
    blocked = { config, pkgs, lib, ... }: {
      virtualisation.vlans = [ 1 ];
      networking = {
        useDHCP = false;
        interfaces.eth1.ipv4.addresses = [{ address = "192.168.1.30"; prefixLength = 24; }];
        firewall.allowedTCPPorts = [ 80 ];
      };
