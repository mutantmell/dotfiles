# Snapshot test for yggdrasil router nftables ruleset
#
# TEMPORARY: Pre-deployment validation. Remove after yggdrasil is successfully
...
#
# Evaluates the actual yggdrasil router6 config (with hardware stubs) and
...
#
# Run: nix build .#checks.x86_64-linux.yggdrasil-firewall-snapshot
#
# To update the golden file after an intentional change:
#   nix-instantiate --eval --strict tests/lib/yggdrasil-firewall-snapshot.nix --arg updateGolden true 2>&1 | \
#     sed 's/^"//' | sed 's/"$//' | sed 's/\\n/\n/g' | sed 's/\\"/"/g' > tests/lib/expected-yggdrasil-firewall.nft
...
let
  ...
  # These match the actual values used in hosts/yggdrasil/default.nix
  host    = { ipv4 = "10.0.11.1"; ipv6 = "fdc6:55f2:0a5e:b::1"; };
  alfheim = { ipv4 = "10.0.11.2"; ipv6 = "fdc6:55f2:0a5e:b::2"; };
  surtr   = { ipv4 = "10.0.100.40"; ipv6 = "fdc6:55f2:0a5e:64::28"; };
  mimir   = { ipv4 = "10.0.11.3"; ipv6 = "fdc6:55f2:0a5e:b::3"; };
  tyr     = { ipv4 = "10.0.11.4"; ipv6 = "fdc6:55f2:0a5e:b::4"; };
  ...
  eval = import (pkgs.path + "/nixos/lib/eval-config.nix") {
    ...
    modules = [
      ../../modules/router6
      {
        ...
        # ================================================================
        # Exact yggdrasil router6 config (from hosts/yggdrasil/default.nix)
...
in
  if updateGolden then ruleset
    ...
    expected = builtins.readFile ./expected-yggdrasil-firewall.nft;
    pass = ruleset == expected;
  in
    if pass then
      pkgs.runCommand "yggdrasil-firewall-snapshot" {} ''
        echo "PASS: yggdrasil nftables ruleset matches golden file"
        echo "PASS" > $out
      ''
    else
      pkgs.runCommand "yggdrasil-firewall-snapshot" {
        ...
        echo "FAIL: yggdrasil nftables ruleset does not match golden file"
        echo ""
        echo "=== DIFF (expected vs actual) ==="
        diff --color=auto "$expectedPath" "$rulesetPath" || true
        echo ""
        echo "To update the golden file, run:"
        echo "  nix-instantiate --eval --strict tests/lib/yggdrasil-firewall-snapshot.nix --arg updateGolden true 2>&1 | \\"
